...
WORKFLOW_RUN_TIMEOUT = timedelta(hours=int(os.getenv("TAPCRAFT_WORKFLOW_RUN_TIMEOUT_HOURS", "12")))
...
DEFAULT_ACTIVITY_MAX_ATTEMPTS = int(os.getenv("TAPCRAFT_DEFAULT_MAX_ATTEMPTS", "10"))

# Default start-to-close timeout for activities.
DEFAULT_ACTIVITY_TIMEOUT_SECONDS = int(os.getenv("TAPCRAFT_DEFAULT_ACTIVITY_TIMEOUT", "300"))

# Absolute ceiling: even if a user sets higher, we cap at this.
MAX_ACTIVITY_RETRY_ATTEMPTS = int(os.getenv("TAPCRAFT_MAX_RETRY_ATTEMPTS", "50"))


# -- Health monitoring --------------------------------------------------------

# An activity retrying more than this many times is considered "stuck".
STUCK_ACTIVITY_THRESHOLD = int(os.getenv("TAPCRAFT_STUCK_THRESHOLD", "10"))
